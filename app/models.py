...
class Job(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    job_number: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
...
class JobItem(Base):
    ...
    product: Mapped[Product | None] = relationship(back_populates="job_items")
...
class Receipt(Base):
    ...
    editable_snapshot: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
